...
class StripeClasses: NSObject {
    ...
    func createCheckout(
        eventId: Int?, orderType:String?, totalUserLoyaltyPoint: String?,
        totalUserSpentAmount:String?, ticketIDs: [CheckoutTicketID]?,
        addOnList: [CheckoutAddonList]?, controller:UIViewController?,
        complition: @escaping (CheckoutId?,Bool,String) -> Void
    ) {
    ...
    func createCharge(amount:Double,cardId:Int,checkoutId:String,controller:UIViewController?,complition: @escaping (CreateCharge?,Bool,String) -> Void ) {
        let req = CreateChargeRequest.init(amount: amount, cardId: cardId, checkoutId: checkoutId)
