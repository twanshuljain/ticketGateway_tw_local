...
class FBProgressView: UIView {
    ...
    @IBInspectable var progressIcon: UIImage = UIImage(named: PROFILE_ICON) ?? UIImage() {
        didSet {
    ...
    private func setupView() {
        ...
        ringProgressView = RingProgressView(
            frame: CGRect(x: 0, y: 0, width: ringView.frame.width, height: ringView.frame.height)
        )
        ringView.addSubview(ringProgressView)
    }
    func setProgress(_ progress: Int,
                     startColor: UIColor = UIColor(named: "EndGradient") ?? UIColor.red,
                     endColor: UIColor = UIColor(named: "EndGradient") ?? UIColor.red,
