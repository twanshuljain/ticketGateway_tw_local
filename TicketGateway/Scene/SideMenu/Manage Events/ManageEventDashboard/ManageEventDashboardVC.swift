...
class ManageEventDashboardVC: UIViewController {
    ...
    func setup() {
        ...
        self.btnSeeMore.setTitles(
            text: "See More",
            font: UIFont.setFont(fontType: .medium, fontSize: .seventeen),
            tintColour: UIColor.setColor(colorType: .tgBlue),
...
extension ManageEventDashboardVC {
    @IBAction private func btnSeeMoreTickets(_ sender: Any) {
        let view = self.createView(storyboard: .manageevent,
                                   storyboardID: .ManageEventTicketSoldVC) as? ManageEventTicketSoldVC
...
extension ManageEventDashboardVC: NavigationBarViewDelegate {
    func navigationBackAction() {
        ...
        let menu = sb.instantiateViewController(withIdentifier: "SideMenuNavigationController") as? SideMenuNavigationController ?? UIViewController()
